import teachersData from '../../chalaoshi_data/teachers.csv'
import gpaData from '../../chalaoshi_data/gpa.json'
...
function getCollegesFromComments() {
  ...
  const teacherData = parseCSV(teachersData)
...
export async function loadData() {
  if (!teachers) {
    teachers = parseCSV(teachersData)
  }
  return { teachers, gpaData }
}

export async function loadComments(college) {
  if (!comments[college]) {
    try {
      const commentData = await import(`../../chalaoshi_data/comment_${college}.csv`)
      comments[college] = parseCSV(commentData.default)
    } catch (error) {
